...
// Modified by Elivoa, remove mongodb dependency. add mysql support.
...
type UploadHandler struct {
	...
	// *** remove mongo db dependency *** use mysql instead
	// Cache  *mc.Conn   // Cache for image thumbnails
	Cache *FakeCache // Cache for image thumbnails
}
...
func (h *UploadHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("----------------------------")
	fmt.Println(r.URL.Path, h.Prefix)
...
func (c *FakeCache) Set(key string) {
	fmt.Println("Fake Cache.Set" + key)
}
func (c *FakeCache) Get(key string) (val string, n1 string, n2 string, e error) {
	fmt.Println("Fake Cache.Set " + key)
	e = errors.New("No such method error!")
	return
}
...
func (s FakeDataStore) Create(*FileInfo, io.Reader) error {
	// Create a new file and return its key
	fmt.Printf("DataStore.Create, %v", 33)
	return nil
}

func (s FakeDataStore) Get(key string) (FileInfo, io.Reader, error) {
	// Get the file blob identified by key
	fmt.Printf("DataStore.Get, %v", key)
	return FileInfo{}, nil, nil
}

func (s FakeDataStore) Delete(key string) error {
	// Delete the file identified by key
	fmt.Printf("DataStore.Delete, %v", key)
	return nil
}

func GetFakeDataStore() DataStore {
